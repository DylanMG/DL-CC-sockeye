...
for(i in unique(all_draws$pop)){
  for(j in c("m1", "m2b")){ #add or adjust the models as seen fit
...
## Population weights with and without informative prior  

LOOIC stacking weights compared between models 4a and 2a to highlight where the informative prior helped in hierarchical models.  

```{r fig4, fig.width=5.62, fig.height=4.215, dpi= 300}
pop_weights_hier <- read.csv(here("output/CV/pop_weights_hier.csv"))

fig_data <- pop_weights_hier |>
...
gridExtra::grid.arrange(patchworkGrob(pw), 
