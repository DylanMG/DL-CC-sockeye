...
subtitle:  |
    ...
    | Paper analysis
author: "W.I. Atlas, D.M. Glaser, B.M. Connors, C.A. Holt, D. Greenberg, D.T. Selbie, S. Cox-Rogers, C. Carr-Harris, E. Hertz, and J.W. Moore."
...
if("m2a" %in% list.files(here("output/model_fits")) & refit == FALSE){
  ...
  m2a <- stan(here("Stan/model2a.stan"), 
              data=stan_data, 
              model_name = "m2a", 
              iter = 4000) #SOMETIMES needs more iter to fit
...
if("m4" %in% list.files(here("output/model_fits")) & refit == FALSE){
  ...
  m4 <- stan(here("Stan/model4_ind-sigs.stan"), 
             data=stan_data, 
             model_name = "m4",  
             iter = 4000)
  saveRDS(m4, file = here("output/model_fits/m4_ind-sigs"))
}
```

```{r diag}
if(FALSE){ #testing stuff while parameterizing models and choosing priors
fit1.summary  <- as.data.frame(rstan::summary(m1)$summary)

bayesAB::plotNormal(exp(-12),exp(3))
bayesAB::plotGamma(3,2)

mcmc_combo(m4, combo = c("dens_overlay", "trace"), 
           pars = c("log_a", "sd_a_pop","sd_sigma_pop", "sigma", "lp__"), 
           #pars = c("log_a", "sd_a_pop", "sigma", "lp__"), 
           gg_theme = legend_none())
fit4.summary  <- as.data.frame(rstan::summary(m4)$summary)
write.csv(fit4.summary, "C:/Users/GLASERD/Desktop/fit-4-reparm-no-region.csv", row.names = F)

m4 <- readRDS(here("output/model_fits/m4_reparm_no-region")) 
m4_ind_sig <- readRDS(here("output/model_fits/m4_reparm_no-region_ind-sigs"))

m4.summary  <- as.data.frame(rstan::summary(m4)$summary) |>
  mutate(parm = rownames(m4.summary))

m4.ind.summary  <- as.data.frame(rstan::summary(m4_ind_sig)$summary)|>
  mutate(parm = rownames(m4.ind.summary))

launch_shinystan(m2a)

#compare smax
Smax_diff <- bind_rows(data.frame(pop = pop_data$pop, 
                                  model = "shared-sig",
                        Smax = slice(m4.summary, grep("Smax", m4.summary$parm))$`50%`, 
                        Smax_LCI = slice(m4.summary, grep("Smax", m4.summary$parm))$`25%`,
                        Smax_UCI = slice(m4.summary, grep("Smax", m4.summary$parm))$`75%`),
                        data.frame(pop = pop_data$pop,
                                   model = "ind-sigs",
                                   Smax = slice(m4.ind.summary, grep("Smax", m4.ind.summary$parm))$`50%`,
                        Smax_LCI = slice(m4.ind.summary, grep("Smax", m4.ind.summary$parm))$`25%`,
                        Smax_UCI = slice(m4.ind.summary, grep("Smax", m4.ind.summary$parm))$`75%`))


ggplot(Smax_diff) +
  geom_point(aes(x = Smax, y = pop, color = model), alpha = 0.5) +
  geom_errorbarh(aes(y = pop, xmin = Smax_LCI, xmax=Smax_UCI, color = model), alpha = 0.5)
}
```
