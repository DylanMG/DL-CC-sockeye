...
fig_1_data <- filter(all_draws, pop %in% c( "Johnston", "Kainet", "Slamgeesh", "Skidegate", "Mathers", "Morice", "Atnarko", "Canoona", "Owikeno")) |>
...
ggplot(fig_1_data, aes(model, log_a)) +
  ...
  geom_text(x=2, y = 3, aes(label = pop))+ #writes it a ton
  theme_bw()
...
ggplot(fig_1_data, aes(model, log_a)) +
  geom_violin(fill = "grey",
              draw_quantiles = 0.5) +
  facet_wrap(~pop) +
  geom_text(x=.2, y = 3, aes(label = pop))+ #MAKE BETTER!
  theme_bw()
...
p <- gridExtra::grid.arrange(patchworkGrob(pw), 
                        left = textGrob(expression("M4a looic stacked weight (m4a vs m2a)"), 
