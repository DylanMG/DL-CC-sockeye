...
if("m4" %in% list.files(here("output/model_fits")) & refit == FALSE){
  ...
  m4 <- stan(here("Stan/model4_reparm_no-region.stan"), 
             data=stan_data, 
             model_name = "m4",  
             iter = 4000)
  saveRDS(m4, file = here("output/model_fits/m4_reparm_no-region"))
}
```

```{r m4 diag}
bayesAB::plotNormal(0,2)
...
m4_reparm <- readRDS(here("output/model_fits/m4_reparm_G3-2")) 
m4 <- readRDS(here("output/model_fits/m4"))

m4_reparm.summary  <- as.data.frame(rstan::summary(m4_reparm)$summary) |>
  mutate(parm = rownames(m4_reparm.summary))

m4.summary  <- as.data.frame(rstan::summary(m4)$summary)|>
  mutate(parm = rownames(m4.summary))

#compare smax
Smax_diff <- data.frame(pop = pop_data$pop, 
                        m4_reparm_Smax = slice(m4_reparm.summary, grep("Smax", m4_reparm.summary$parm))$mean, 
                        m4_Smax = slice(m4.summary, grep("Smax", m4.summary$parm))$mean) |>
  mutate(perc_diff = round((m4_Smax-m4_reparm_Smax)/m4_Smax, 2))
