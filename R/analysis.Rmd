...
#avg_S <- #filter data, group by pop, summarise
...
write.csv(M4_Smsy, here("output/data/M4a_Smsy_med.csv"), row.names = FALSE)
#print figure from file. figure was made by Jon Hart at Wild Salmon Centre
```
### Figure 1  

```{r fig1, fig.width=5.62, fig.height=4.215, dpi= 300}
fig_1_data <- filter(all_draws, pop %in% c("Mathers", "Kitlope", "Morice", "Atnarko", "Canoona", "Damdochax", "Koeye", "Long", "Owikeno")) |>
  left_join(pop_data, by = "pop") |>
  mutate(data_quality = case_when(pop %in% c("Mathers") ~ "low", #10,  
                                  pop %in% c("Atnarko", "Canoona", "Damdochax", "Koeye") ~ "medium", #20-40: 38, 31, 22, 26
         pop %in% c("Kitlope", "Morice", "Long", "Owikeno") ~ "high"),#40+: 46, 42, 56, 56
         data_quality = factor(data_quality, levels = c("low", "medium", "high"))) 
  
fig_1_data |>
  select(pop, region.x, data_quality) |>
  distinct() |>
  arrange(region.x, data_quality)

ggplot(fig_1_data, aes(log_a, color = model)) +
...
ggplot(fig_1_data, aes(model, log_a)) +
...
p <- ggplot(filter(all_draws, model == "m4a"), aes(region, log_a)) +
  geom_violin(fill = "grey",
              draw_quantiles = 0.5) +
...
p1 <- ggplot(fig4_data,aes(sr_pairs, m4a)) +
  theme_classic() +
  geom_point(aes(color = region)) +
  geom_hline(yintercept = 0, 
             lty = 2) +
  annotate("text", x = 5, y = 1.1, label = "(A)") +
  theme(legend.position = "bottom", 
        legend.title = element_text(size = 9),
        legend.text = element_text(size = 7),
...
p2 <- ggplot(filter(fig4_data, est_origin != "both"), aes(est_origin, m4a)) +
  ...
  #annotate("text", x= .52, y = 1.1, label = "(B)") +
  theme_classic() +
  theme(text = element_text(family="serif"),
        legend.position= "bottom", 
        legend.title = element_text(size=9),
...
p3 <- ggplot(fig4_data, aes(region, m4a)) +
      geom_violin(#aes(fill=est_origin), 
                  alpha = 0.5, 
                  draw_quantiles = 0.5) +
  geom_jitter(width = 0.25) +
    geom_hline(yintercept = 0, 
             lty = 2) +
  #annotate("text", x= .6, y = 1.1, label = "(C)") +
  theme_classic() +
  theme(text = element_text(family="serif")) +
...
p4 <- ggplot(fig4_data, aes(water_clarity, m4a)) +
  geom_violin(#aes(fill=est_origin), 
              alpha = 0.5, 
              draw_quantiles = 0.5) +
  geom_jitter(width = 0.25) +
  ...
  geom_hline(yintercept = 0, 
             lty = 2) +
  #annotate("text", x= .6, y = 1.1, label = "(D)") +
  theme_classic() +
  theme(text = element_text(family="serif")) + 
  scale_fill_viridis_d() +
...
p <- gridExtra::grid.arrange(patchworkGrob(pw), 
                        left = textGrob(expression("Support for including informative prior (m4a vs m2a"), 
