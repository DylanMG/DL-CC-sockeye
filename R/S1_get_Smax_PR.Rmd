---
title: "Supplement 1 - calculating SmaxPR and SDs"
author: "Dylan M. Glaser"
...
# $S_{MAX_{PR}}$
Here we estimate maximum spawners based on photosynthetic rates ($S_{MAX_{PR}}$) for 96 lakes without empirical estimates of mean daily photosynthetic rate ($PR_{MEAN}$) in [Atlas et al. 2020](https://aslopubs.onlinelibrary.wiley.com/doi/10.1002/lno.11448) (hereon the L&O paper). We use table S5 from the supplementary materials reported in the bottom of the previous link to calculate $S_{MAX_{PR}}$ estimates then aggregate them into sockeye populations (since a single population can use multiple lakes for rearing) to be re-analysed from a preprint (heron the [BioRxiv paper](https://www.biorxiv.org/content/10.1101/2021.03.22.436497v1)).  

$S_{MAX_{PR}}$ estimates will ultimately be transformed and used as priors on the beta parameter in a Ricker stock-recruit model. We use empirical $S_{MAX_{PR}}$ estimates from table S1 from the L&O paper, then use modeled estimates of $S_{MAX_{PR}}$ calculated here as priors. We assign error around $S_{MAX_{PR}}$ based on lake clarity categories and $PR_{MEAN}$ estimates shown in figure 3a of the L&O paper.  

In an ideal world, we'd have empirical estimates of $S_{MAX_{PR}}$ error that describe sampling error within lakes when estimating $PR_{MEAN}$, but that doesn't exist. The L&O figure S3 highlights the variability in empirical $PR_{MEAN}$ by water clarity group. Since water clarity was selected in all top models in the L&O paper, and $PR_{MEAN}$ estimates aren't yet scaled up to lake size, we feel using the variability in Figure 3a strikes a balance between overstepping inference and simplicity.  

## The data  

### PR and PR derived variables  
There's several levels of PR variables we need to keep track of, that are described on p. 2207 of the L&O paper:  
...
- `lake_data` is a table that was used in the analysis of 70 sockeye populations in the BioRxiv paper, which corresponds to the file `lakePR_final.txt` Brendan Connors gave me (Dylan) when I started playing with this analysis.  
...
```{r load-L&O-data, echo = TRUE}
lnos1_skinny <- read.csv(here("data_input/lno_s1.csv"), header = TRUE) |> 
...
lnos5 <- read.csv(here("data_input/lno_s5.csv"), header = TRUE) |>
...
```{r load-SK-data}
sockeye_pops <- read.table(here("data_input/lakePR_final2.txt"), header = TRUE) |>
...
```{r check-pops-that-need-match}
print(sockeye_pops$lake[which(!(sockeye_pops$lake %in% all_lakes$lake))])
```
...
```{r L&O-leftovers}
print(all_lakes$lake[which(!(all_lakes$lake %in% sockeye_pops$lake))])
```

belong to any of these populations:  
```{r sockeye-pops}
print(unique(sockeye_pops$lake))
```
...
```{r read-SEs}
water_SDs <- read.table(file = here("data_input/water_SDs.txt"), header = TRUE) |>
...
```{r table-s1}
original_S1 <- read.csv(here("data_input/lno_s1.csv"), header = TRUE) |>
  mutate(lake = tolower(gsub(" Lake| Lakes", "", Lake))) |>
